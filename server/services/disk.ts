...
export async function saveRolling(
  ...
  const matches = entries.filter((e) => e.startsWith(prefix) && e.endsWith(".jpg"));
  ...
  if (matches.length > maxSnapshots) {
    ...
    await Promise.all(toDelete.map(({ name }) => unlink(join(savePath, name)).catch(() => {})));
  }
}
...
export async function clearCache(savePath: string): Promise<void> {
  ...
  await Promise.all(entries.map((name) => unlink(join(savePath, name)).catch(() => {})));
}
