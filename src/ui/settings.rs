...
pub fn show(ui: &mut egui::Ui, state: &mut AppState) {
    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        egui::CollapsingHeader::new("Districts")
            .default_open(false)
            .show(ui, |ui| show_districts(ui, state));

        ui.separator();

        egui::CollapsingHeader::new("Cameras")
            .default_open(false)
            .show(ui, |ui| show_cameras(ui, state));

        ui.separator();

        egui::CollapsingHeader::new("Display")
            .default_open(true)
            .show(ui, |ui| show_display(ui, state));

        ui.separator();

        egui::CollapsingHeader::new("Disk Save")
            .default_open(true)
            .show(ui, |ui| show_disk_save(ui, state));
    });
}

// ── Section helpers ───────────────────────────────────────────────────────────

fn show_districts(ui: &mut egui::Ui, state: &mut AppState) {
    ...
    for district in &districts {
        let mut checked = state.pending_config.selected_districts.contains(district);
...
fn show_cameras(ui: &mut egui::Ui, state: &mut AppState) {
    ...
    ui.label(
        egui::RichText::new("🔼/🔽 to reorder · uncheck to hide · takes effect on Apply")
            .small()
    ...
    let active_ids = ordered_camera_ids(&all_cameras, &selected_districts, &camera_order);
    ...
    for (pos, &cam_id) in active_ids.clone().iter().enumerate() {
        ...
        ui.horizontal(|ui| {
            ...
            // Visibility checkbox.
            let mut visible = !state.pending_config.hidden_camera_ids.contains(&cam_id);
...
/// Build the ordered list of camera IDs for the active districts.
/// Hidden cameras are included so they keep their position.
fn ordered_camera_ids(
    all_cameras: &[crate::camera::CameraInfo],
    selected_districts: &std::collections::BTreeSet<String>,
    camera_order: &[u32],
) -> Vec<u32> {
    ...
    if camera_order.is_empty() {
        ...
        sorted.sort_by(|a, b| a.district.cmp(&b.district).then(a.name.cmp(&b.name)));
...
fn show_display(ui: &mut egui::Ui, state: &mut AppState) {
    ...
    ui.add(egui::Slider::new(
        &mut state.pending_config.refresh_interval_secs,
        10u32..=300,
    ));
    ...
    ui.label("Columns:");
    ui.add(egui::Slider::new(
        &mut state.pending_config.column_count,
        1usize..=6,
    ));

    ui.add_space(4.0);
    ui.label("Grid spacing:");
    ui.add(egui::Slider::new(&mut state.pending_config.grid_spacing, 0.0..=16.0).suffix(" px"));
    ...
    ui.add_enabled_ui(state.pending_config.show_camera_titles, |ui| {
        ui.add(
            egui::Slider::new(
                &mut state.pending_config.camera_title_font_size,
                9.0f32..=24.0,
            )
            .integer()
            .suffix(" pt"),
    ...
    ui.add_enabled_ui(state.pending_config.show_camera_titles, |ui| {
        let [r, g, b] = &mut state.pending_config.camera_title_rgb;
        ui.add(egui::Slider::new(r, 0u8..=255).integer().text("R"));
        ui.add(egui::Slider::new(g, 0u8..=255).integer().text("G"));
        ui.add(egui::Slider::new(b, 0u8..=255).integer().text("B"));
        ui.horizontal(|ui| {
            ui.label("Preview:");
    ...
    let [r, g, b] = &mut state.pending_config.app_background_rgb;
    ui.add(egui::Slider::new(r, 0u8..=255).integer().text("R"));
    ui.add(egui::Slider::new(g, 0u8..=255).integer().text("G"));
    ui.add(egui::Slider::new(b, 0u8..=255).integer().text("B"));
    ui.horizontal(|ui| {
...
fn show_disk_save(ui: &mut egui::Ui, state: &mut AppState) {
    ui.checkbox(
    ...
    ui.add(egui::Slider::new(
        &mut state.pending_config.max_snapshots,
        1usize..=20,
    ));
