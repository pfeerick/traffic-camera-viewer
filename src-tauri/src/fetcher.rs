...
/// Outcome of a single camera image fetch.
#[derive(Debug, Clone, Serialize)]
#[serde(tag = "type", rename_all = "snake_case")]
pub enum CameraImageFetchOutcome {
    /// Image has changed since last fetch; jpeg_b64 contains the base64-encoded JPEG.
    Changed { hash: u64, jpeg_b64: String },
    /// Image is identical to the previous fetch (hash match).
    Unchanged { hash: u64 },
}

/// Fetch a single camera image, optionally save to disk, and return the result
/// as a base64-encoded JPEG string for transfer over Tauri IPC.
pub async fn fetch_camera_image(
    ...
    // Validate that the bytes are a valid JPEG before sending.
    image::load_from_memory_with_format(&bytes, image::ImageFormat::Jpeg)
        .map_err(|e| FetchError::Decode(e.to_string()))?;

    let jpeg_b64 = base64::engine::general_purpose::STANDARD.encode(&bytes);
    Ok(CameraImageFetchOutcome::Changed { hash, jpeg_b64 })
}
